...
public class FileMatchReaderTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		FileMatchReader fileMatchReader = new FileMatchReader(playerSet);

		File matchFile = new File(matchdir, "Match.33941143263.xml");
		
		match33941143263 = fileMatchReader.readMatch(new MatchSet("Breakthroughholes_2007_R5_D1", 2007, 5,
				1, 1, gameSet.getGame("Breakthroughholes")), 
				matchFile);		

		matchFile = new File(matchdir, "Match.3390055120.xml");

		match3390055120 = fileMatchReader.readMatch(new MatchSet(
				"Blocksworld-Obfuscated_2007_R1_D1", 2007, 1, 1, 1, 
				gameSet.getGame("Blocksworld-Obfuscated")), 
				matchFile);		
	}
	...
	public void testGetPlayers() {
		...
		assertEquals(players.get(0), playerSet.getPlayer("FLUXPLAYER"));
		assertEquals(players.get(1), playerSet.getPlayer("CLUNEPLAYER"));
		
		
		players = match3390055120.getPlayers();
		
		assertEquals(players.size(), 1);
		assertEquals(players.get(0), playerSet.getPlayer("THE-PIRATE"));
	}
