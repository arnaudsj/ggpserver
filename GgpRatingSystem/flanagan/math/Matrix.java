...
*
*   Copyright (c) April 2004, July 2007   Michael Thomas Flanagan
*
*   PERMISSION TO COPY:
...
public class Matrix{
        ...
        public Matrix(Matrix bb){
		    ...
		    this.matrix = (double[][])bb.matrix.clone();
		    this.permutationIndex = (int[])bb.permutationIndex.clone();
	    	...
	    	for (int i=0;i<n;i++) {
        		...
        		if (big == 0.0D){
        		    if(!this.supressErrorMessage){
            		    ...
            		    /* --- BEGIN added by MG --- */
            		    throw new IllegalArgumentException("Attempted LU Decomposition of a singular matrix in Matrix.luDecomp()");
            		    /* --- END   added by MG --- */
         	        }
		        ...
		        for(int p=0;p<this.numberOfRows-1;p++){
			        for(int q=p+1;q<this.numberOfRows;q++){
				        ...
				        if (i > 4 && (Math.abs(this.eigenValues[p]) + scaledOffDiagonal) == Math.abs(this.eigenValues[p]) && (Math.abs(this.eigenValues[q]) + scaledOffDiagonal) == Math.abs(this.eigenValues[q])){
				            ...
				            amat[p][q] = 0.0;
					        for(int j=0;j<=p-1;j++) rotation(amat, tanHalfRotationAngle, sElement, j, p, i, q);
        ...
        private void eigenSort(){
	        ...
	        this.sortedEigenValues = (double[])this.eigenValues.clone();
	        this.sortedEigenVector = (double[][])this.eigenVector.clone();
