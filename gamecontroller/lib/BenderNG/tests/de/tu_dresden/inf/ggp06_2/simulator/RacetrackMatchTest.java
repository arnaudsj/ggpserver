...
public class RacetrackMatchTest {

    private GameManager gameManager = new GameManager();
    private Match racetrackMatch = null;
    private Theory theory = null;

    private static final Logger logger = Logger.getLogger( RacetrackMatchTest.class );
    private static final Atom a50 = new Atom("50");
    private static final Atom a100 = new Atom("100");

    @Before
    public void setUp(){
        ExpressionList chessRules = Parser.parseFile("./testdata/games/Racetrack.kif");
        Game           game       = gameManager.getGameByGDL( chessRules.toString() );
        AbstractStrategy dummy    = new RandomStrategy(game, "white");
        racetrackMatch = new Match("0", dummy, "0");
        theory = racetrackMatch.strategy.getGame().getTheory();
    }
    ...
    public void turnMaking(){
        ...
        GameState initial = racetrackMatch.getState();
        logger.info( " before " );
        for (Atom role : racetrackMatch.strategy.getGame().getRoleNames()){
            try {
                racetrackMatch.strategy.getGame().getLegalMoves(
