...
<jsp:useBean id="pager"
	     class="tud.ggpserver.formhandlers.ShowDBFilter" scope="page">
    <c:catch>
	<jsp:setProperty name="pager" property="show_matches" />
	<jsp:setProperty name="pager" property="page" />
    </c:catch>
</jsp:useBean>


<c:set var="title">DB Filter</c:set>


<% if (session.getAttribute("filter") == null)
      session.setAttribute("filter", pager.getFilter());      
   else
      pager.setFilter(session.getAttribute("filter"));
   %>
...
<c:if test="${pager.show_matches == 1}">
<% pager.init(); %>
